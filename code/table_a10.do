...
// Save 
esttab  m2_fossil_state m2_ratio_state m2_coal_state m2_coal m2_gas_state m2_ng m2_wind_state m2_wind  ///
		using "$results_path/table_a10.tex", ///
		label booktabs b(%20.3f) se(%20.3f) eqlabels(none) alignment(S S)  ///
		keep(*in_state_demand*)  ///
		star(* 0.10 ** 0.05 *** 0.01) ///
		stats(me_at_mean se_at_mean KP_Wald_F N , ///
		fmt(0 0 %3.2f ) layout( "@" "@" "\multicolumn{1}{c}{@}" "\multicolumn{1}{c}{@}") ///
		label("Out-of-State REC Demand (TWh)" "~" "\hline Kleibergen-Paap F-Stat" "\hline Observations")) ///
		se f nonumbers mtitles("Out-of-State REC Demand" "Add Gas Generation" "Add Coal Generation" "Add Gas and Coal") substitute(\_ _) replace
