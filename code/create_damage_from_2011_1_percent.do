...
//Combine with emissions as a result of Actual and Marginal RPS increases
	...
	merge 1:1 StateFIPS CountyFIPS using "$data_path/rps_induced_emission_changes_2011.dta"
	
	keep if _merge==3
	drop _merge

//Save Intermediate Point 
	save "$temp_path/temp_2011.dta", replace
...
//Generate Actual Differences
	...
	foreach pollutant in `nox' {
		gen nox_diff_`pollutant'= `pollutant'*nox_diff_tons
		drop `pollutant'
	}

	foreach pollutant in `so2' {
		gen so2_diff_`pollutant'= `pollutant'*so2_diff_tons
		drop `pollutant'
	}

	foreach pollutant in `pm_25' {
		gen pm_25_diff_`pollutant'= `pollutant'*pm_25_diff_tons
		drop `pollutant'
	}

	foreach pollutant in `pm_10' {
		gen pm_10_diff_`pollutant'= `pollutant'*pm_10_diff_tons
		drop `pollutant'
	}

	foreach pollutant in `voc' {
		gen voc_diff_`pollutant'= `pollutant'*voc_diff_tons
		drop `pollutant'
	}

	foreach pollutant in `nh3' {
		gen nh3_diff_`pollutant'= `pollutant'*nh3_diff_tons
		drop `pollutant'
	}
//Save Full Actual Damage Dataset
	save "$data_path/damages_transpose_full.dta", replace

	clear all
	use "$data_path/damages_transpose_full.dta"
	keep *so2_s_* *nox_s_* *pm_25_s_* *pm_10_s_* *voc_s_* *nh3_s_*
	
	gen order = _n
	
	qui ds *so2_s_* *nox_s_* *pm_25_s_* *pm_10_s_* *voc_s_* *nh3_s_*
	...
	keep if order==1
//Reshape Dataset
	capture reshape long  sum_so2_diff_so2_s_@ sum_nox_diff_nox_s_@ sum_pm_25_diff_pm_25_s_@ sum_pm_10_diff_pm_10_s_@ sum_voc_diff_voc_s_@  sum_nh3_diff_nh3_s_, i(year) j(FIPS) string

	split FIPS, parse("_") gen(fips)
	destring fips1, gen(StateFIPS)
	destring fips3, gen(CountyFIPS)
	drop FIPS fips*
	sort StateFIPS CountyFIPS year
	order StateFIPS CountyFIPS year
	
	rename sum_* *
	
	rename *_s_* *
	
	rename so2_diff_so2 so2_damages
	rename nox_diff_nox nox_damages
	rename nh3_diff_nh3 nh3_damages
	rename voc_diff_voc voc_damages
	rename pm_25_diff_pm_25 pm_25_damages
	rename pm_10_diff_pm_10 pm_10_damages
	
	egen total_damages = rowtotal(so2_damages nox_damages nh3_damages voc_damages pm_25_damages pm_10_damages)

	save "$data_path/county_damages_2011.dta", replace


	clear all
	use "$temp_path/temp_2011.dta"
	...
	local state_lower  az	ca	co	ct	de	il  ia	ks  me	md	ma	mn  mo  mt	nv	nh	nj  nm	ny	nc oh or pa ri  tx wi	


	foreach st in `state_lower' {     
		//Load Dataset from Above
			use "$temp_path/temp_2011.dta", clear
		...
		//Save State Level and US Level Damage datasetfrom ALL sources by Pollutant in the proper format
			save "$data_path/state_us_damages_from_`st'_rps_inc_2011.dta", replace
	...
	//Append the datasets
	clear all
	use "$data_path/state_us_damages_from_az_rps_inc_2011.dta"

	local state_lower  ca	co	ct	de	il  ia	ks  me	md	ma	mn  mo  mt	nv	nh	nj  nm	ny	nc oh or pa ri  tx wi	

	foreach  st in `state_lower' {
		append using "$data_path/state_us_damages_from_`st'_rps_inc_2011.dta"
	}

	save "$data_path/rps_inc_damage_2011.dta", replace

	erase  "$temp_path/temp_2011.dta"
